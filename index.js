...
// declare constants
const intToOutcome = ['A wins!', 'B wins!', 'Draw'];
const {standardUnit} = reach;
const defaults = {defaultFundAmt: '10', defaultWager: '0', defaultNum: '0', standardUnit};
...
class Shared extends React.Component {
    ...
    async getGuess() {
        const guess = await new Promise(resolveGuessP => {
            this.setState({view: 'GetNum', playable: true, resolveGuessP});
        });
    ...
    seeActual(magicNum) { 
      ...
      this.setState({view: 'SeeActual', num: parseInt(magicNum)}); 
    }
    async getNum() {
      const tempNum = Math.floor(Math.random() * 50);
      return tempNum;
    }
}
